...
public class ReportController {
    ...
    public SearchReportResponse runReport() {
        ...
        int totalCount = (int) this.searchService.searchProductCount("all").get("all"); // Assuming search service returns a map with total count for "all"
        response.setProductCount(totalCount);

        Map<String, Integer> termHits = searchService.searchProductCount("Cool", "Kids", "Amazing", "Perfect");
        response.setSearchTermHits(termHits);
